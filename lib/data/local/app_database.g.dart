...
class AppUser extends DataClass implements Insertable<AppUser> {
  ...
  AppUser copyWith({int? id, String? name, String? email, String? password}) =>
      AppUser(
        id: id ?? this.id,
        name: name ?? this.name,
...
class $ItemsTable extends Items with TableInfo<$ItemsTable, AppItem> {
  @override
  final GeneratedDatabase attachedDatabase;
  final String? _alias;
  ...
  @override
  VerificationContext validateIntegrity(Insertable<AppItem> instance,
      {bool isInserting = false}) {
    final context = VerificationContext();
    final data = instance.toColumns(true);
    if (data.containsKey('id')) {
  ...
  Set<GeneratedColumn> get $primaryKey => {id};
  @override
  AppItem map(Map<String, dynamic> data, {String? tablePrefix}) {
    final effectivePrefix = tablePrefix != null ? '$tablePrefix.' : '';
    return AppItem(
      id: attachedDatabase.typeMapping
          .read(DriftSqlType.int, data['${effectivePrefix}id'])!,
...
class AppItem extends DataClass implements Insertable<AppItem> {
  final int id;
  final String title;
  ...
  AppItem copyWithCompanion(ItemsCompanion data) {
    return AppItem(
      id: data.id.present ? data.id.value : this.id,
...
class ItemsCompanion extends UpdateCompanion<AppItem> {
  final Value<int> id;
  ...
  static Insertable<AppItem> custom({
    Expression<int>? id,
...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  @override
  List<DatabaseSchemaEntity> get allSchemaEntities => [users, items];
}
...
class $$ItemsTableTableManager extends RootTableManager<
    ...
    (AppItem, BaseReferences<_$AppDatabase, $ItemsTable, AppItem>),
    AppItem,
    PrefetchHooks Function()> {
